...
export const createPost = async (req, res) => {
    try {
        ...
        if(!text && !img){
            return res.status(400).json({error: 'Pust must have text or image'})
        }
...
export const deletePost = async (req, res) => {
    try{
      ...
      res.status(200).json({message: 'Post deleted successfully'});
        ...
        console.log('Error in delepost controller:', error);
...
export const getFollowingPosts = async (req, res) => {
    try {
       ...
       const feedPost = await Post.find({user: {$in: following}})
         ...
         console.log('Error in getFollwingPost controller:', error)
         res.status(500).json({error: 'Internal server error'})
    }
